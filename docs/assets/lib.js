// ArcadeWire static deployment:
// - Encodes/decodes the same human-friendly code format.
// - Uses local device storage as the "server" so GitHub Pages can demo the UX.

// ------------------------
// Wordlist (256 entries)
// ------------------------
const WORDS = [
  "amber","angel","apple","arbor","atlas","audio","awoke","baker","banjo","beach","berry","bison","blaze","bloom","brave","bream",
  "brick","bring","bunny","cabin","cacao","camel","candy","caper","cedar","chain","chalk","charm","chess","cider","civic","clean",
  "clear","cliff","climb","cloud","coast","comet","coral","crown","dance","daisy","delta","diner","drape","drift","dream","dwell",
  "eager","eagle","early","earth","ember","enjoy","equal","ethos","evoke","fable","faith","fancy","ferry","field","flame","flare",
  "flora","focus","forge","fresh","frost","fruit","gamma","gazer","giant","glade","glide","glint","glory","grace","grain","grand",
  "grant","graph","grass","green","grove","guide","happy","harpy","haven","honey","honor","hotel","house","human","ideal","ivory",
  "jewel","judge","kayak","kiosk","kitty","knack","knoll","label","laser","later","laugh","layer","lemon","light","lilac","limit",
  "linen","lodge","logic","lunar","lyric","macro","magic","mango","maple","march","match","medal","merge","merry","metal","meter",
  "micro","mirth","model","moody","mossy","motor","mount","mural","naval","noble","noise","north","novel","oasis","ocean","olive",
  "orbit","order","other","panel","paper","pearl","peony","phase","piano","pilot","plain","plaid","plaza","pride","prime","print",
  "prism","prize","quiet","quill","radar","radio","rally","rapid","raven","realm","relay","ridge","river","roast","robin","robot",
  "royal","route","rural","sable","salad","salsa","scale","scene","scent","scope","scout","sense","serve","shade","share","sheen",
  "shift","shiny","shore","short","sight","silky","skate","slate","slope","smile","smoke","solar","solid","solve","sonic","sound",
  "south","spark","spice","spike","spool","spray","squad","stack","stage","stair","stare","steam","steel","stone","storm","story",
  "strap","straw","sugar","sunny","surge","swift","table","tango","taper","tempo","theme","think","tiger","toast","today","topic",
  "torch","total","trace","trail","treat","trend","truly","tulip","tuned","unity","urban","value","vapor","vivid","vocal","wagon",
  "water","weave","wheat","whirl","widen","wiser","witty","world","xenon","yacht","yearn","yield","young","zebra","zesty","zonal",
  "zippy","zowie","bevel","birch","bliss","broad","cable","cargo","catch","cheer","choir","craft","crane","creek","crisp","cumin",
  "curry","dandy","deter","dingo","doubt","easel","elite","enter","epoch","fever","final","finer","fixer","given","golem","great",
  "hatch","helix","jazzy","jolly","kneel","knelt","latch","loved","maker","miner","mixer","ninth","oaken","offer","pacer","pause",
  "piece","piper","pivot","place","punch","quick","raise","react","right","rinse","risen","saint","score","sleep","sober","space",
  "spare","spend","spoke","stand","start","stint","stoke","stout","tacit","taken","taste","tease","thank","thick","tight","token",
  "trust","uncle","upper","waltz","worry",
];
...
const STORAGE_KEY = "arcadewire:v1:exchanges";
...
export function createExchange() {
  ...
  const exchangeId = bytesToBase64Url(idBytes);
  const code = encodeIdBytesToCode(idBytes);
  ...
  return { exchangeId, code, expiresAt };
}
...
export function decodeCode(input) {
  ...
  if (!suffixBytes || suffixBytes.length !== SUFFIX_BYTES) return { ok: false, reason: "invalid_suffix" };
  ...
  return { ok: true, checksumPresent, normalized, exchangeId: bytesToBase64Url(idBytes) };
}

export function resolveExchange(exchangeId) {
  const store = loadStore();
  const ex = store[exchangeId];
  if (!ex) return { ok: false, state: "missing", message: "That code doesn’t match an active ArcadeWire." };

  const now = Date.now();
  if (ex.state === "pending" && now >= ex.expiresAt) ex.state = "expired";
  if (ex.state === "expired") {
    store[exchangeId] = ex;
    saveStore(store);
    return { ok: false, state: "expired", message: "That ArcadeWire expired. Create a new one and try again." };
  }
  if (ex.state === "resolved") return { ok: false, state: "resolved", message: "That ArcadeWire was already used." };

  ex.state = "resolved";
  ex.resolvedAt = now;
  store[exchangeId] = ex;
  saveStore(store);
  return { ok: true, state: "resolved", resolvedAt: now };
}
...
function encodeIdBytesToCode(idBytes) {
  const word1 = WORDS[idBytes[0]];
  const word2 = WORDS[idBytes[1]];
  const suffixBytes = idBytes.slice(2);
  const suffix = base32Encode(suffixBytes).slice(0, SUFFIX_CHARS);
  const check = B32[checksum5(idBytes) & 31];
  return `${word1}-${word2}-${suffix}${check}`.toLowerCase();
}
...
function formatCoords2() {
  return [
    [20, 8],[19, 8],[18, 8],[17, 8],[16, 8],[15, 8],[14, 8],[13, 8],[8, 20],[8, 19],[8, 18],[8, 17],[8, 16],[8, 15],[8, 14],
  ];
}
...
function formatBits(fmt5) {
  let data = fmt5 << 10;
  const gen = 0x537;
  for (let i = 14; i >= 10; i--) if (((data >> i) & 1) === 1) data ^= gen << (i - 10);
...
function qrAlnumDataCodewordsV1(text) {
  ...
  let pad = 0;
  while (bytes.length < 19) bytes.push(pad++ % 2 === 0 ? 0xec : 0x11);
...
function polyMul(p, q) {
  ...
  for (let i = 0; i < p.length; i++) for (let j = 0; j < q.length; j++) out[i + j] ^= gfMul(p[i], q[j]);
  return out;
}
